...
class PrimaryRendererStreamAos : public Renderer, IntegratorBase
{
private:
    struct State
    {
...
public:
    PrimaryRendererStreamAos(const ref<const Scene>& scene, const ref<IntersectorStreamAos<streamSize>>& intersector, const Props& props)
        : Renderer(props)
    {
        this->scene = scene;
    ...
    void render(const Camera* camera, FrameBuffer* frameBuffer, Props& stats)
    {
        if (frameBuffer->getSize() != imageSize)
            throw std::invalid_argument("PrimaryRendererStreamAos: framebuffer size mismatch");
