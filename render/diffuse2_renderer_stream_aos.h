...
class Diffuse2RendererStreamAos : public Renderer, IntegratorBase
{
private:
    struct StateIo
    {
        RayHitStreamAos<streamSize> ray;
        RayStreamChannelAos<float, streamSize> L;
...
public:
    Diffuse2RendererStreamAos(const ref<const Scene>& scene, const ref<IntersectorStreamAos<streamSize>>& intersector, const Props& props)
        : Renderer(props)
    {
        this->scene = scene;
    ...
    void render(const Camera* camera, FrameBuffer* frameBuffer, Props& stats)
    {
        if (frameBuffer->getSize() != imageSize)
            throw std::invalid_argument("Diffuse2RendererStreamAos: framebuffer size mismatch");
    ...
    void renderTile(const Vec2i& tileId, int tid)
    {
        ...
        for (;;)
        {
            ...
            for (int i = 0; i < missCount; ++i)
            {
                ...
                float L = stateI->L[pathId];
                L += throughput * ((depth > 0) ? 0.5f : 1.f); // with MIS weight

                int pixelId = stateI->pixelId[pathId];
                if (accum)
                    frameBuffer->getColor().add(pixelId, Vec3f(L));
                else
                    frameBuffer->getColor().set(pixelId, Vec3f(L));
            }
